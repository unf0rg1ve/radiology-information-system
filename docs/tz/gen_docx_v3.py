"""Generate RIS MVP TZ v3 as DOCX."""
...
r = tp.add_run('ТЕХНИЧЕСКОЕ ЗАДАНИЕ')
r.bold = True; r.font.size = Pt(22); r.font.color.rgb = BLUE

sp = doc.add_paragraph()
sp.alignment = WD_ALIGN_PARAGRAPH.CENTER
r2 = sp.add_run('MVP радиологической информационной системы (RIS)')
r2.bold = True; r2.font.size = Pt(16)

doc.add_paragraph()
mp = doc.add_paragraph()
mp.alignment = WD_ALIGN_PARAGRAPH.CENTER
mp.add_run('Версия 3.0  |  2026-06-13\nРеспублика Казахстан\n\n').font.size = Pt(12)
...
def flush_code(lines):
    ...
    shd = OxmlElement('w:shd')
    shd.set(qn('w:val'), 'clear'); shd.set(qn('w:color'), 'auto'); shd.set(qn('w:fill'), 'EEF3F8')
    pPr.append(shd)
    r = para.add_run(text)
...
with open('ris-mvp-tz-v3.md', encoding='utf-8') as f:
    lines = f.readlines()
...
while i < len(lines):
    ...
    if m:
        doc.add_heading(m.group(2), level=min(len(m.group(1)), 4)); i += 1; continue
    ...
    if mb or mn:
        ...
        if len(match.group(1)) >= 4: para.paragraph_format.left_indent = Cm(1.5)
...
doc.save('ris-mvp-tz-v3.docx')
print('Saved: ris-mvp-tz-v3.docx')
